...
| `evidence_scope` | immutable | `stage_id`, `source_set_revision_id`, `input_version` (для аудита), `content_hash`, `created_by`, `created_at` | снимок области доказательств (R01-01): создаётся до начала проверки и больше не меняется; одинаковый состав даёт тот же `content_hash` и ту же строку. Номер версии снимка актуальность не подтверждает: покрытие событий проверяется по составу (`state-machines.md` §1.1) |
...
| Таблица | Класс | Ключевые колонки | Ограничения |
|---|---|---|---|
| `recognition_run` | frozen-after | `document_revision_id`, `tender_id`, `engine` (`rdweb_export`/`rdweb_api`/`text_layer`/`local_ocr`), `engine_schema_version`, `source_artifact_sha256`, `source_artifact_name`, `status` (`queued`/`running`/`complete`/`partial`/`failed`), `pages_total`, `pages_recognized`, `quality`, `failure_code`, `failure_detail`, `supersedes_run_id`, `row_version` | после финального статуса неизменна; новый OCR — новая строка (A10). Пара (`document_revision_id`, `source_artifact_sha256`) уникальна среди незавершившихся отказом прогонов: один архив — один прогон, после `failed` повтор разрешён. `complete` невозможен, пока `pages_recognized < pages_total` (CHECK, I18) |
...
| `evidence_fragment` | immutable | `tender_id`, `source_unit_type` (`recognition_run`/`communication`/`transcript_revision`), `source_unit_id`, `run_id` или `transcript_segment_id` или `communication_id`, `document_revision_id`, `origin` (см. ниже), `fragment_kind`, `fragment_key`, `external_block_id`, `ordinal`, `page_index`, `bbox_norm numeric[4]`, `bbox_space`, `shape_type`, `polygon_norm`, `rotation`, `text`, `text_sha256`, `derived_model_ref`, `external_crop_url`, `warnings` | ID портала стабилен; текст не редактируется; единица источника — основа фильтра области (ADR-008) |
| `fragment_index_state` | derived | `fragment_id`, `index_system` (`localai`/`portal_fts`), `index_version`, `status`, `indexed_at` | можно удалить и пересобрать (I15, A42). **Не создана на этапе 04**: до этапа 05 (X-04) у неё нет ни писателя, ни читателя, а её форма зависит от контракта индекса — заводится вместе с индексацией |
