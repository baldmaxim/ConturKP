...
Текущий этап: 04 RDWeb — READY_FOR_REVIEW (объём сужен владельцем до импорта результатов распознавания, D-010)
...
Ближайшее действие: независимое ревью этапа 04 (`docs/stages/04-report.md`); слияние `stage-00`…`stage-04` в `main` — по решению владельца
...
| 04 RDWeb | READY_FOR_REVIEW | цепочка `89024f8` → `bfbd924` → `2e8a4b1` → `d64be89` → `eecdfb6` (база `301e7f2`) | `docs/stages/04-report.md` | — | Q-02 (другие варианты схемы экспорта); X-05 — только автоматическая постановка задач, вне объёма этапа (D-010) |
